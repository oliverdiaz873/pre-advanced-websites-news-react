...
export const TabletNav = () => {
  return (
    <nav aria-label="Navegación de Tablet" className="hidden md:block lg:hidden">
        ...
        {navItems.map((item) => (
          <li key={item.name}>
            <NavLink
              to={item.path}
              className={({ isActive }) =>
                `block rounded px-2 py-1.5 text-[14px] text-black transition-all duration-300 dark:text-gray-300 ${
                  isActive
